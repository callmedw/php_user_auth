<?php
  ...
  if ($filter=='all') {
    ...
    $tasks = getCompleteTasks($user['id']);
  } else {
    $filter = 'incomplete';
    $tasks = getIncompleteTasks($user['id']);
  }
      ...
      <table class="items">
        <?php
          if (!empty($tasks) && $tasks != null) {
            echo "<tr><th>Status</th><th>Title</th><th>Action</th></tr>";
            ...
            echo "You do not have any $filter tasks at this time.";
          }
        ?>
      </table>
    </div>
